...
<div id="payment-overlay" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.38);backdrop-filter:blur(2px);z-index:60;align-items:center;justify-content:center;padding:20px;">
    <div id="payment-overlay-card" style="min-width:280px;max-width:420px;width:100%;background:#fff;border-radius:16px;box-shadow:0 24px 50px rgba(15,23,42,.24);padding:18px 20px;text-align:center;border:1px solid #e2e8f0;">
        ...
        <div id="payment-overlay-title" style="font-size:16px;font-weight:800;color:#18324d;">Memproses pembayaran...</div>
...
<h2 style="margin-top:0;">Total Bayar: Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</h2>

<p style="color:#555;">Invoice: {{ $pembayaran->invoice ?? '-' }}</p>
...
@if($paymentDriver === 'midtrans')
<button id="pay-button" style="padding:12px 18px;border:0;border-radius:8px;background:#111827;color:#fff;cursor:pointer;">Bayar Sekarang</button>
@else
<p style="margin:12px 0;font-size:13px;color:#059669;background:#ecfdf5;padding:10px;border-radius:6px;border:1px solid #6ee7b7;">
    <strong>✓ Akses checkout Pakasir sudah siap.</strong> Setelah Anda klik tombol di bawah, Anda akan diarahkan ke halaman Pakasir untuk menyelesaikan pembayaran. Jika halaman menampilkan "Sandbox Payment Simulator", berarti akun Pakasir masih dalam mode sandbox. Hubungi support Pakasir untuk upgrade ke production.
</p>
...
<form id="cancel-form" method="POST" action="{{ route('payment.cancel', $pembayaran->id) }}" style="display:inline-block;margin-left:10px;">
@csrf
...
@if($paymentDriver === 'midtrans')
<p style="margin-top:10px;font-size:13px;color:#6b7280;">Jika memilih Virtual Account, popup bisa tetap terbuka (status pending). Klik tombol X di popup, lalu Batalkan Transaksi untuk ganti metode.</p>
<p style="margin-top:6px;font-size:13px;color:#6b7280;">Silakan pilih metode pembayaran yang tersedia dan ikuti petunjuk pada layar hingga transaksi selesai.</p>
@endif
...
const finishUrl = "{{ route('payment.finish') }}";
const csrfToken = "{{ csrf_token() }}";
const cancelForm = document.getElementById('cancel-form');

document.getElementById('pay-button').onclick = function () {

showPaymentOverlay('Memproses pembayaran...', 'Mohon tunggu, sistem sedang memverifikasi transaksi Anda.');

snap.pay('{{ $snapToken }}', {

onSuccess: async function(result){
showPaymentOverlay('Pembayaran berhasil', 'Tagihan Anda telah dibayar dan sedang diproses.');
console.log(result);
...
body: JSON.stringify({
order_id: result.order_id || "{{ $pembayaran->invoice }}"
})
});
} catch (error) {
...
onClose: function(){
showPaymentOverlay('Popup ditutup', 'Jika ingin ganti metode, klik Batalkan Transaksi lalu Bayar Sekarang lagi.');
