...
<div class="password-wrap">
    <div class="hero-card">
        ...
        <h2 class="mt-1 text-3xl font-black">Setel Password</h2>
        <p class="mt-2 max-w-3xl text-sm text-white/82">Anda login menggunakan Google. Silakan atur password untuk alternatif login dengan email & password.</p>
    </div>

    <div class="panel space-y-6">
        <div class="info-box text-sm">
            <strong>Informasi:</strong> Setelah mengatur password, Anda bisa login menggunakan email dan password ini, atau tetap menggunakan Google.
        </div>
        ...
        <form action="{{ route('user.settings.store-password') }}" method="POST" class="space-y-4">
            @csrf

            <div>
                <label class="field-label">Email</label>
                <input type="email" name="email" value="{{ $user->email }}" readonly class="field-input bg-slate-50 text-slate-500 cursor-not-allowed">
            </div>
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" required class="field-input" placeholder="Ulangi password">
            </div>

            <div class="actions">
                <button type="submit" class="btn-primary">Setel Password</button>
